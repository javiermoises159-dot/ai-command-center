...
| Cerebras / Mistral / Cloudflare Workers AI | Tres proveedores propios sobre el mismo adapter (`real/hosted-compat.ts`), con su URL incorporada. Cerebras: `CEREBRAS_API_KEY` + `CEREBRAS_MODEL` (p. ej. `gpt-oss-120b`). Mistral: `MISTRAL_API_KEY` + `MISTRAL_MODEL` (p. ej. `mistral-small-latest`). Cloudflare: `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_MODEL` (p. ej. `@cf/meta/llama-3.1-8b-instruct`). Se asume plan gratuito (0 USD); si pasas a uno de pago, defínelo en `MADRE_PRICES_JSON`. Los ids de modelo envejecen: cópialos del panel de cada servicio | `POST {base}/chat/completions` | `Authorization: Bearer` |
