...
export function defaultToolSpecs(): ToolSpec[] {
  return [
    ...
    tool({
      id: 'news.gdelt',
      name: 'Noticias recientes (GDELT)',
      category: 'research',
      description: 'Busca noticias recientes de miles de medios en muchos idiomas. Devuelve titulares y enlaces.',
      capabilities: ['research.web'],
      provider: 'GDELT',
      input: { query: str('Tema a buscar.', true) },
      output: { articles: str('Lista de titulares con su enlace, medio, idioma y fecha.') },
      locality: 'remote',
      version: '1.0.0',
      timeoutMs: SEC(20),
      limits: { maxCallsPerRun: 10, maxInputBytes: 2_000 },
      status: 'NOT_CONNECTED',
      detail: 'Listo pero apagado: el servidor lo activa con NEWS_ENABLED=true (por defecto en producción). API pública y gratuita, sin clave.',
    }),
    tool({
