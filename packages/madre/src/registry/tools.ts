...
export function defaultToolSpecs(): ToolSpec[] {
  return [
    ...
    tool({
      ...
      detail: 'No hay ningún proveedor de búsqueda configurado. La investigación se basa en el conocimiento del modelo y lo indica.',
    }),
    tool({
      id: 'web.fetch',
      ...
      output: { text: str('Texto extraído.'), fetchedAt: str('Marca de tiempo ISO.') },
      locality: 'remote',
      ...
      limits: { maxCallsPerRun: 30, maxInputBytes: 4_000 },
      status: 'PLANNED',
      detail: 'Todavía no existe. Necesita una lista de permitidos y el bloqueo de redes privadas antes de ser segura.',
    }),
    tool({
      ...
      capabilities: ['research.web'],
      provider: 'Wikipedia',
      input: { title: str('Título del artículo.', true) },
      output: { extract: str('Sección introductoria.') },
      locality: 'remote',
      version: '0.1.0',
      timeoutMs: SEC(15),
      limits: { maxCallsPerRun: 30, maxInputBytes: 2_000 },
      status: 'PLANNED',
      detail: 'Todavía no hay conector.',
    }),
