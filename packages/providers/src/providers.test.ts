...
describe('ProviderRegistry', () => {
  ...
  it('lists planned providers so the roadmap is visible without pretending', () => {
    ...
    assert.deepEqual(
      ...
      ['anthropic', 'cerebras', 'cloudflare', 'gemini', 'mistral', 'mock', 'nvidia', 'ollama', 'openai', 'openai-compatible', 'openrouter', 'sambanova'],
    );
    ...
    assert.equal(described.filter((d) => d.availability === 'unconfigured').length, 10);
  });
